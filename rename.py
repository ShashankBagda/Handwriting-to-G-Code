...
for i, filename in enumerate(file_list):
    ...
    if i < len(num_arr):
        new = 'Output/' + num_arr[i] + '.png'
        os.rename(old, new)
    elif i > 40:
        new_al = 'Output/' + alphabet_arr[j] + '.png'
        os.rename(old, new_al)
        j = j + 1
    else:
        new_al = 'Output/' + special_char[k] + '.png'
        os.rename(old, new_al)
        k = k + 1
